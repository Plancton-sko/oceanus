...
Scope {
    ...
    function applyWallpaper(path) {
        ...
        Quickshell.execDetached([root.homeDir + "/scripts/theme_switcher", "wallpaper", path, root.colorschemeMode]);
    ...
    function setColorschemeMode(mode) {
        ...
        if (root.currentThemeMode === "wallpaper" && root.currentWallpaperPath !== "") {
            Quickshell.execDetached([root.homeDir + "/scripts/theme_switcher", "wallpaper", root.currentWallpaperPath, mode]);
        } else {
    ...
    FileView {
        ...
        path: "file://" + root.homeDir + "/dev/rice/nixos/doty/wabi/presets"
        watchChanges: true
    ...
    Process {
        ...
        command: [root.homeDir + "/scripts/presets_lister"]
        running: false
                        ...
                        if (event.key === Qt.Key_Right || event.key === Qt.Key_L) {
                            ...
                            if (root.lastFocus === "wallpaper" && wallpaperTimeline.count > 0) {
                                ...
                                if (root.presetFocusIndex === 0) {
                                    ...
                                    if (p) {
                                        Quickshell.execDetached([root.homeDir + "/scripts/theme_switcher", "preset", p.name]);
                                        win.closePopup();
                                    }
                                }
                            }
