...
hl.bind(
	mainMod .. " + M",
	hl.dsp.exec_cmd(
		"hyprctl clients | grep -iq 'class: .*spotify' && hyprctl dispatch 'hl.dsp.focus({ window = \"class:^[Ss]potify$\" })' || uwsm app -- spotify"
	)
)
hl.bind(
	mainMod .. " + Y",
	hl.dsp.exec_cmd(
		"hyprctl clients | grep -iq 'class: .*freetube' && hyprctl dispatch 'hl.dsp.focus({ window = \"class:^[Ff]ree[Tt]ube$\" })' || uwsm app -- freetube"
	)
)
