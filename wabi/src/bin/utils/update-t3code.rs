...
fn get_latest_t3code_version() -> Option<String> {
    let mut args = vec![
        "-s",
        "-H",
    ...
    args.push("https://api.github.com/repos/pingdotgg/t3code/releases/latest");

    let output = Command::new("curl").args(&args).output().ok()?;

    if !output.status.success() {
        eprintln!("Failed to fetch T3 Code release info from GitHub API.");
        return None;
    }
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("Checking for T3 Code updates...");

    let desktop_nix_path = Path::new("modules/hosts/oceanus/packages/desktop.nix");
    ...
    // Locate T3 Code block
    let start_idx = content
        .find("# -- T3 Code --")
        .ok_or("Cannot find T3 Code header")?;
    let t3_block = &content[start_idx..];

    let current_version =
        extract_field(t3_block, "version = \"", "\";").ok_or("Cannot find current version")?;

    println!("Current local version: {}", current_version);

    let latest_version = match get_latest_t3code_version() {
        Some(v) => v,
        None => {
            eprintln!("Warning: Could not check for T3 Code updates. Skipping update check.");
            return Ok(());
        }
    };
    ...
    if latest_version == current_version {
        println!("T3 Code is already up to date!");
        return Ok(());
    }
    ...
    let appimage_url = format!(
        "https://github.com/pingdotgg/t3code/releases/download/v{}/T3-Code-{}-x86_64.AppImage",
    ...
    let end_idx = content[start_idx..]
        .find("})")
        .ok_or("Cannot find end of T3 Code block")?
        + start_idx
        + 2;
    let old_block = &content[start_idx..end_idx];

    let current_hash =
        extract_field(old_block, "sha256 = \"", "\";").ok_or("Cannot find current hash")?;
    ...
    println!(
        "Successfully updated desktop.nix T3 Code to version {}",
        latest_version
    );
    ...
    if should_commit {
        ...
        let status = Command::new("git")
            .args(["add", "modules/hosts/oceanus/packages/desktop.nix"])
            .status()?;
        if !status.success() {
        ...
        let commit_msg = format!("chore: auto-update T3 Code to version {}", latest_version);
        let status = Command::new("git")
