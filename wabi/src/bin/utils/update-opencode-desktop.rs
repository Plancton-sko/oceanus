...
fn get_latest_opencode_version() -> Option<String> {
    let mut args = vec![
        "-s",
        "-H",
    ...
    args.push("https://api.github.com/repos/anomalyco/opencode/releases/latest");

    let output = Command::new("curl").args(&args).output().ok()?;

    if !output.status.success() {
        eprintln!("Failed to fetch OpenCode release info from GitHub API.");
        return None;
    }
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("Checking for OpenCode Desktop updates...");
    ...
    // Locate OpenCode Desktop block
    let start_idx = content
        .find("# -- OpenCode Desktop --")
        .ok_or("Cannot find OpenCode Desktop header")?;
    let opencode_block = &content[start_idx..];

    let current_version = extract_field(opencode_block, "version = \"", "\";")
        .ok_or("Cannot find current version")?;
    let current_hash =
        extract_field(opencode_block, "sha256 = \"", "\";").ok_or("Cannot find current hash")?;

    println!("Current local version: {}", current_version);

    let latest_version = match get_latest_opencode_version() {
        Some(v) => v,
        None => {
            eprintln!(
                "Warning: Could not check for OpenCode Desktop updates. Skipping update check."
            );
            return Ok(());
        }
    };
    ...
    if latest_version == current_version {
        println!("OpenCode Desktop is already up to date!");
        return Ok(());
    }
    ...
    let download_url = "https://opencode.ai/download/stable/linux-x64-deb";
    ...
    let end_idx = content[start_idx..]
        .find("})")
        .ok_or("Cannot find end of OpenCode Desktop block")?
        + start_idx
        + 2;
    ...
    if should_commit {
        ...
        let commit_msg = format!(
            "chore: auto-update OpenCode Desktop to version {}",
            latest_version
        );
        let status = Command::new("git")
