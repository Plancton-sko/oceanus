...
fn get_latest_tldraw_version() -> Option<String> {
    let mut args = vec![
        "-s",
        "-H",
    ...
    args.push("https://api.github.com/repos/tldraw/tldraw-offline/releases/latest");

    let output = Command::new("curl").args(&args).output().ok()?;

    if !output.status.success() {
        eprintln!("Failed to fetch Tldraw Offline release info from GitHub API.");
        return None;
    }
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("Checking for Tldraw Offline updates...");
    ...
    // Locate Tldraw Offline block
    let start_idx = content
        .find("# -- Tldraw Offline --")
        .ok_or("Cannot find Tldraw Offline header")?;
    let tldraw_block = &content[start_idx..];

    let current_version =
        extract_field(tldraw_block, "version = \"", "\";").ok_or("Cannot find current version")?;
    let current_hash =
        extract_field(tldraw_block, "sha256 = \"", "\";").ok_or("Cannot find current hash")?;

    println!("Current local version: {}", current_version);

    let latest_version = match get_latest_tldraw_version() {
        Some(v) => v,
        None => {
            eprintln!(
                "Warning: Could not check for Tldraw Offline updates. Skipping update check."
            );
            return Ok(());
        }
    };
    ...
    if latest_version == current_version {
        println!("Tldraw Offline is already up to date!");
        return Ok(());
    }
    ...
    let new_url = format!(
        "https://github.com/tldraw/tldraw-offline/releases/download/v{}/tldraw-offline-linux-x86_64.AppImage",
        latest_version
    );
    ...
    let end_idx = content[start_idx..]
        .find("})")
        .ok_or("Cannot find end of Tldraw Offline block")?
        + start_idx
        + 2;
    ...
    if should_commit {
        ...
        let commit_msg = format!(
            "chore: auto-update Tldraw Offline to version {}",
            latest_version
        );
        let status = Command::new("git")
